...
void FileDialog::init(){
  ...
  dir->addEntries(Filesystem::getDirectories(cwd_));
  ...
  file->addEntries(Filesystem::getFiles(cwd_));
...
void FileDialog::changeDir(GuiElement* elem){
  ...
  cwd_ += "/"+dir;
  dirs->clear();
  dirs->addEntries(Filesystem::getDirectories(cwd_));
  //update file list
  ListBox* files = dynamic_cast<ListBox*>(dia->getElement(1));
  files->clear();
  files->addEntries(Filesystem::getFiles(cwd_));
}
