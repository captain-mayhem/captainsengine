...
public:
  class Node;
  typedef CGE::Ptr<Node> NodeRef;
  typedef CGE::Ptr<StreetInfo> StreetRef;
  ...
  NodeRef addNode(const CGE::Vec3f& position);
  StreetRef addStreet(const StreetInfo& info);
  void addStreetSegment(NodeRef from, NodeRef to, StreetRef street);
