...
|                                                                  |
| Copyright 2005 by Marco Staginski and Stefan Ruff                |
...
Known bugs:
The character must run with the same speed regardless of the current framerate. So, if the framerate
goes down, the distance the character moves between two frames gets bigger in order to keep the speed
constant. If the framerate falls under 40 fps, the distance becomes so huge, that sometimes the
character is in front of a wall in one frame and in the next frame, he is already completly behind. This means,
that no collision with the wall is detected and it looks as if you can walk through it. If the framerate
falls unter 20 fps, the collision detection fails completely. This can cause invalid states that can lead
to segmentation faults. However, if the computer is good enough, i.e. the game runs with more than 50
fps, the stepsize is small enough that no errors will occur. Becuause this is true for every computer 
that is reasonably up-to-date, I consider this behaviour not as a real bug.
...
Currently, we are two programmers, but there is always need for more.
