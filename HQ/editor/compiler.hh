...
#include "../heroquest/vector.hh"
#include "../heroquest/field.hh"
#include "../heroquest/door.hh"
...
public:
   ...
   */
  void addRoom(const Vector2D& pos, short id);
  //! Sets a wall at a Field
  /*! \param pos the position of the Field
   *  \param d the Direction of the wall
   */
  void setWall(const Vector2D& pos, Direction d);
  ...
  void write(string filename);
	 ...
	 */
	void addMonster(const Vector2D& pos, char monster[3]);
	
	//! Add a furniture
	/*! \param pos the position of the furniture
	 *  \param furniture the furniture type
	 */
	void addFurniture(const Vector2D& pos, char furniture[3]);
	
  //! Add an overlay
	/*! \param pos the position of the overlay
	 *  \param overlay the overlay type
	 */
	void addOverlay(const Vector2D& pos, char overlay[3]);
	
	//! read in monster types
	void init();

  //! Add a script
  void addScript(char ident, const Vector2D& pos);
private:
  ...
  Field **map_;
  //! the room structure
  vector< vector<Vector2D> > rooms_;
  //! the hero starting positions
  vector<Vector2D> starts_;
